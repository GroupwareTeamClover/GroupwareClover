...
//         ChatRoomDTO room = chatDAO.getRoomById(roomSeq, empSeq);
//         if (room == null) {
//             // 채팅방이 존재하지 않거나 사용자가 채팅방에 속해있지 않은 경우
//             return;
//         }
        
//         if (room.getRoomType().equals("private")) {
//             // 1:1 대화인 경우
//             List<Integer> members = chatDAO.getRoomMembers(roomSeq);
//             for (int memberSeq : members) {
//                 chatDAO.removeUserFromRoom(roomSeq, memberSeq);
...
//             }
//             // 채팅방 삭제
//             chatDAO.deleteRoom(roomSeq);
//         } else {
//             // 그룹 채팅인 경우
//             chatDAO.removeUserFromRoom(roomSeq, empSeq);
//             EmployeeDTO user = chatDAO.getEmployeeName(empSeq);
//             if (user != null) {
...
//         chatDAO.deleteChatHistory(roomSeq, empSeq);
//     }

//     public void toggleNotifications(int roomSeq, int empSeq, boolean enabled) {
//         String enabledStr = enabled ? "Y" : "N";
//         chatDAO.updateNotificationSettings(roomSeq, empSeq, enabledStr);
//     }
