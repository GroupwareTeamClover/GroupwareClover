...
const CreateBoard = () => {
    ...
    useEffect(() => {
        axios.get(`http://${baseUrl}/boardlist/members`).then((resp) => {
            setMembers(resp.data);
            console.log(resp.data);
        });
        axios.get(`http://${baseUrl}/boardlist/depts`).then((resp) => {
            console.log(resp.data);
            setDepts(resp.data);
        });
    }, []);
    ...
    const handleCreate = () => {
        if(title.trim() == ''){
            ...
            axios.post(`http://${baseUrl}/boardlist`, {
                title : title,
                type : boardType,
        ...
        <div className={styles.container}>
            ...
            <div className={styles.memberBox}>
                <div className={styles.memberLetter}>
                    사용자 추가
                    <div className={styles.memberLetterCaution}>주의 : 전사 게시판 선택 시 그룹게시판 접근 허용자 목록은 모두 사라집니다.</div>
                </div>
